...
由于没有分隔符，数字编码成字母可能有多种编译结果，例如 11 既可以看做是两个 'a' 也可以看做是一个 'k' 。但 10 只可能是 'j'（h之后是j, 不是i ） ，因为0不能编译成任何结果
...
func solve(nums string) int {
	...
	step 3：对于只有一种译码方式的，选上种dp[i−1]即可，对于满足两种译码方式（10，20不能）则是dp[i−1]+dp[i−2]（搞清楚哪些条件只能一种翻译，哪些是可以有两种翻译方法：前面等于0，前面是1或者2）
