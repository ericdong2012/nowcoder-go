...
func FindNumsAppearOnce(array []int) []int {
	// write code here
	//将数组进行分组异或
	//保证相同元素在同一组，不同元素不在同一组
	//异或操作，相同为0，不同为1
	//先把所有的值进行异或，得到的结果就是两个不同的值异或的结果
	//找到这个值中有1的位，说明这个位置上一个是0，一个是1，例如6(110),4(100),异或010
	//第二位为1,根据这个位置来对所有的值进行分组，相同的值肯定分到同一组，不同的值不在同一组
	//6和4必定不在同一组,相同的元素必在同一组
	ans := 0
	for _, v := range array {
		ans ^= v
	}
