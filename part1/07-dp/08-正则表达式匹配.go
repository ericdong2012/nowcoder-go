...
step 2： （初始条件） 首先，毋庸置疑，两个空串是直接匹配，因此dp[0][0]=truedp[0][0]=truedp[0][0]=true。然后我们假设str字符串为空，那么pattern要怎么才能匹配空串呢？答案是利用'*'字符出现0次的特性。遍历pattern字符串，如果遇到'*'意味着它前面的字符可以出现0次，要想匹配空串也只能出现0，那就相当于考虑再前一个字符是否能匹配，因此dp[0][i]=dp[0][i−2]。
...
然后考虑'*'出现的情况：
pattern[j - 2] == '.' || pattern[j - 2] == str[i - 1]：即pattern前一位能够多匹配一位，可以用'*'让它多出现一次或是不出现，
因此有转移方程dp[i][j]=dp[i−1][j] ∣∣ dp[i][j−2]
...
func match(str string, pattern string) bool {
	...
	dp[0][0] = true // pattern为空，此时str字串也为空
	for i := 0; i <= m; i++ {
		for j := 1; j <= n; j++ {
			if pattern[j-1] != '*' {
				// 两个字符相等，或是pattern串中为'.'即可匹配 因此最后一位匹配，即查看二者各自前一位是否能完成匹配
